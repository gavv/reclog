...
use crate::status::*;
use crate::term::{AnsiStripper, InputMode};
use clap::Parser;
use exec::Command;
...
fn main() {
    ...
    if term::is_tty(&stdio::stdin()) {
        if let Err(err) = term::set_input_mode(&stdio::stdin(), InputMode::Canon) {
    ...
    let pty_write_fd = match pty_proc.dup_fd() {
        Ok(fd) => fd,
        Err(err) => {
    ...
    let pty_read_fd = match pty_proc.dup_fd() {
        Ok(fd) => fd,
        Err(err) => {
            eprintln!("reclog: can't duplicate master: {}", err);
            process::exit(EXIT_FAILURE);
    ...
    let pty_reader = match InterruptibleReader::new(pty_read_fd) {
    ...
    let stdin_reader = Arc::new(match InterruptibleReader::new(io::stdin()) {
        Ok(reader) => reader,
        Err(err) => {
